...
// Bull Board (optional) - only if installed and Redis is available
try {
    const { createBullBoard } = require('@bull-board/express');
    const { BullMQAdapter } = require('@bull-board/api/bullMQAdapter');
    const { matchingQueue } = require('./queues/queue');

    // Only set up Bull Board if queue is available
    if (matchingQueue) {
        ...
        console.log('Bull Board not available (Redis disabled)');
    }
} catch (err) {
