...
async function placeNewUser(userId, sponsorId) {
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        try {
            ...
            // take first 16 hex chars -> 8 bytes -> 64-bit
            const prefix = h.slice(0, 16);
            const lockKey = BigInt('0x' + prefix);
            await tx.$executeRaw`SELECT pg_advisory_xact_lock(${lockKey})`;
        } catch (e) {
            // If raw lock fails (e.g., not Postgres or tx doesn't support raw), continue without lock but risk race.
        }
